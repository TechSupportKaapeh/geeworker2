...
| M.9.0c | **`s2-pasada-v2`**: estadísticas **sólo** por pasada, ráster mensual, y el umbral de cobertura al leer. Convive con `s2-mensual-v1` | worker, Geocore | 🟡 **la mitad del worker, hecha** el 2026-09-25 (geeworker2#74, `DECISIONS #69`): la receta existe, está verificada contra GEE y **NO es la vigente**; más el arreglo del filtro de nubes que `#67` dejó anotado, medido y sin efecto en las parcelas reales. **Falta la mitad de Geocore**: `/api/measurements` tiene que agregar, con `mensual` por defecto, que es lo que hace que el cambio de vigente no rompa el panel |
