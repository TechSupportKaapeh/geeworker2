...
- **T-7 (2026-09-20, M.8.1)** — **El token de mapa es de un tenant.** `GET /api/maps/token` exige `X-Tenant-ID` —la autorización la hace `TenantMiddleware`, el mismo control que el resto de la API— y firma el claim `tenant_id`; el tileserver rechaza con 403 cualquier COG que no cuelgue de `tenants/{ese id}/`. Cierra el único camino por el que se podían leer datos de otro tenant sin pasar por Geocore. Ver **T-7** en la tabla del tileserver y `DECISIONS #42` de Geocore.
...
| **T-3** | A10 | **Los assets listados *dentro* del MosaicJSON no pasan por el filtro.** El `path_dependency` valida la URL del documento, pero `cogeo-mosaic` abre los assets que ese documento lista tal como vengan, incluidos `http://` a la red interna. Es T-2 una capa más adentro | ⚠️ **Abierto, con salida decidida el 2026-09-25** (`DECISIONS #64`): **se borra el router `/mosaic`**, no se validan los assets. Nadie usa MosaicJSON —verificado contra el panel, `/piloto`, `check_prod` y el worker— y `#31` ya lo decía: es superficie muerta que carga un hallazgo. Misma cura que **W-3**. Hasta que se haga, lo contiene que sólo `worker-rw` escriba en el bucket. **Desde M.8.1 pesaba más**: el tenant se compara contra la URL del documento y no contra los assets que lista |
...
| **T-7** | A01 | **El token de mapa no decía de qué tenant era.** Validados firma, emisor, audiencia y `type: map-access`, se servía **cualquier** COG del bucket, y la key viaja a la vista en la URL del tile (`?url=s3://…`). Un usuario con su token —legítimo y vigente— y la key de otro tenant veía los rásters de ese otro tenant. Es el hallazgo rojo de la revisión del 2026-09-12, y no se podía cerrar hasta que **todo** lo que el worker escribe colgara de `tenants/{t}/` (M.6.2b) | ✅ **Cerrado 2026-09-20** (M.8.1; `DECISIONS #42` de Geocore). El token lleva `tenant_id` y el `path_dependency` exige `s3://{bucket}/tenants/{ese tenant}/`, **con la barra**. Un token sin el claim da 403: no se toleran los viejos. Verificado con la app entera montada, y con un escalón nuevo de `check_prod.py` contra el deploy |
...
|---|---|---|---|---|
| A10/A01 | tileserver | **T-3**: los assets del MosaicJSON no pasan por el filtro anti-SSRF **ni por la comparación de tenant** (M.8.1) | Alto **si** el mosaico pasa a servirse por HTTP | ✅ Decidido 2026-09-25: **borrar el router `/mosaic`** (`DECISIONS #64`). Falta hacerlo |
...
| A04 | Geocore | Rate limiting en el **login y la edge function `create-user`**. La API quedó cubierta en M.8.3 (`DECISIONS #44`) | Medio | ✅ Decidido 2026-09-25 (`DECISIONS #47` de Geocore): **riesgo aceptado por escrito**. Supabase Auth ya limita el login y `create-user` autoriza antes de tocar nada (N-4) |
| A09 | Geocore | ~~Audit log de acciones privilegiadas~~ | — | ✅ M.8.4 (`DECISIONS #45`). La migración se aplicó el 2026-09-25. **Sin pantalla a propósito** (`DECISIONS #47` de Geocore): se consulta por SQL hasta que alguien la pida con un caso concreto |
