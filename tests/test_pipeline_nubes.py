...
@pytest.mark.gee
def test_la_erosion_de_v1_achica_el_descarte(gee_inicializado):
    # v1 erosiona 2 px desde `DECISIONS #45`. Acá se compara contra la máscara sin
    # erosión, que es la de la capa vieja: sin ella, cada píxel de nube suelto de
    # s2cloudless se convierte en un círculo de 50 m al dilatar.
    import ee

    roi, escena = _escena(ee)
    sin_erosion = dataclasses.replace(RECETA_VIGENTE, nubes_erosion_px=0)

    info = ee.Dictionary({
        "sin": _fracciones(ee, nubes.componentes(escena, sin_erosion), roi, 10),
        "v1": _fracciones(ee, nubes.componentes(escena, RECETA_VIGENTE), roi, 10),
    }).getInfo()

    # La erosión solo puede sacar descarte, nunca sumar.
    assert info["v1"]["descarte"] < info["sin"]["descarte"]
    # Y no toca las nubes en sí: lo que cambia es lo que se dilata.
    assert info["v1"]["nube"] == info["sin"]["nube"]
